...
(function () {
  ...
  function renderQualifiedList() {
    ...
    arr.slice(0, 8).forEach((entry, idx) => {
      const li = document.createElement("li");
      li.innerHTML = `<span class="rank">#${idx + 1}</span><span class="flag">${entry.country.emoji}</span>${entry.country.name}<span class="wins">${entry.wins} win${entry.wins > 1 ? "s" : ""}</span>`;
      dom.qualifiedList.appendChild(li);
    });
  }
